...
from scipy.ndimage import median_filter  # Importa il filtro mediano
...
# Filtro Mediano
def apply_median_filter(X, size=2):
    X_filtered = np.array([median_filter(x.reshape(28, 28), size=size).flatten() for x in X])
    return X_filtered
...
# Funzione per la classificazione con il rilevatore e filtro mediano
def classify_with_detector(model, X, detector, threshold=0.5, apply_filter=False):
    ...
    if np.any(adversarial_indices):
        X_adversarial = X[adversarial_indices]
        if apply_filter:
            X_adversarial = apply_median_filter(X_adversarial)  # Applica il filtro mediano
        corrected_preds = model.predict(X_adversarial)
    else:
        corrected_preds = []
...
# Funzione per eseguire l'attacco e confrontare le accuratezze
def test_and_compare_attack(attack_method, attack_name, epsilons, apply_filter=False):
    scores_no_detector = []
    scores_with_detector = []
    ...
    for epsilon in epsilons:
        ...
        final_preds = classify_with_detector(model, perturbed_images, ensemble_detector, apply_filter=apply_filter)
        final_accuracy = accuracy_score(y_test, final_preds)
        scores_with_detector.append(final_accuracy)

        print(f"Accuratezza senza rilevatore dopo l'attacco '{attack_name}' con epsilon={epsilon}: {score}")
        print(f"Accuratezza con rilevatore dopo l'attacco '{attack_name}' con epsilon={epsilon}: {final_accuracy}")

    return scores_no_detector, scores_with_detector, final_preds
...
for attack_method, attack_name in attacks:
    ...
    scores_no_detector, scores_with_detector, final_preds = test_and_compare_attack(attack_method, attack_name, epsilons, apply_filter=True)
...
for attack_name in results:
    ...
    plt.plot(epsilons, scores_with_detector, 'g*-', label=f'{attack_name} con rilevatore e filtro mediano')
    plt.xlabel('Epsilon')
...
if final_preds is not None:
    final_accuracy = accuracy_score(y_test, final_preds)
    print(f"Accuratezza finale del modello con rilevatore e filtro mediano dopo tutti gli attacchi: {final_accuracy}")
