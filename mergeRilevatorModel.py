...
# Funzione per la classificazione con il rilevatore
def classify_with_detector(model, X, detector, threshold=0.5):
...
def test_and_compare_attack(attack_method, attack_name, epsilons):
    scores_no_detector = []
    ...
    for epsilon in epsilons:
        ...
        final_preds = classify_with_detector(model, X_test, ensemble_detector)
...
for attack_method, attack_name in attacks:
    ...
    scores_no_detector, scores_with_detector, final_preds = test_and_compare_attack(attack_method, attack_name, epsilons)
